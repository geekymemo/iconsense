...
async function initializeIconSense(context: vscode.ExtensionContext, diagnostics: vscode.DiagnosticCollection) {
    try {
        ...
        if (IconSenseState.ready) {
            const shouldRestorePanel = context.globalState.get<boolean>('iconsense.panelOpen');
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 2 Panel komutu
    context.subscriptions.push(
        vscode.commands.registerCommand('iconsense.openPicker', async () => {
            if (!IconSenseState.ready) {
            ...
            if (editor) {
                await StandaloneScanner.scanStandaloneDocument(editor.document);
            }

            IconPickerPanel.createOrShow(context, context.extensionUri, vscode.ViewColumn.Beside, false);
        })
    );
    // Panel komutu - Aktif editor değişince standalone scan
    context.subscriptions.push(
        vscode.window.onDidChangeActiveTextEditor(async (editor) => {
            if (!editor) return;

            const wasStandalone = StandaloneScanner.hasActiveStandaloneIcons();

            await StandaloneScanner.scanStandaloneDocument(editor.document);

            const isStandalone = StandaloneScanner.hasActiveStandaloneIcons();

            // SADECE DURUM DEĞİŞTİYSE refresh
            if (IconPickerPanel.currentPanel && wasStandalone !== isStandalone) {
                IconPickerPanel.currentPanel.refresh();
            }
        })
    );
    ...
    const scheduleScan = (uri: vscode.Uri) => {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                ...
                if (force) {
                    lastForceTime = now;
                } // force kullanıldıysa zamanı güncelle
    ...
    const checkContext = () => {
        ...
        IconPickerPanel.createOrShow(context, context.extensionUri, vscode.ViewColumn.Beside, true);
    };
